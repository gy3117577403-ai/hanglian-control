...
export class AuthService {
  ...
  mockLogin(userId: string): MockSession {
    ...
    if (!user) throw new NotFoundException('Mock user was not found.');
    return {
  ...
  mockMe(request: unknown): MockSession {
    const user = resolveMockUserFromRequestLike(
      request as Parameters<typeof resolveMockUserFromRequestLike>[0],
    );
    return {
      token: tokenForUser(user),
      user,
