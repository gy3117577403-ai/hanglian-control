# 项目状态

当前版本：V2.3 现场知识库演示版。

## 已完成

- V0.1：平板端生产计划资料管控原型。
- V0.2：前端优先走后端 Mock API。
- V0.3：上传、预览、查询留痕、异常反馈基础流程。
- V0.4：数据库接入准备，不连接真实数据库。
- V0.5：本地文件资料管理。
- V0.6：资料版本管理、审计、metadata。
- V0.7：Prisma Repository 草稿、迁移预览、seed dry-run。
- V0.8A：Sealos PostgreSQL 测试库只读验证准备。
- V0.9：本地安全基线提交。
- V1.0-V1.3：暖色立体工业平板 UI、预览体验、现场交互和文件健康 API。
- V1.4：演示资料生成、文件流自检、上传体验复测、预览边界提示、现场模式。
- V1.5：LAN API 自动识别、局域网开发脚本、网络诊断面板、现场走查面板、演示准备检查、平板访问文档。
- V1.6：系统信息面板、演示说明面板、演示界面状态重置、走查结果复制、统一空状态/错误状态、演示版 release check。
- V1.7：演示工具菜单、演示数据管理、演示前检查、演示资料说明、后续路线面板、README 截图占位。
- V1.8：冻结前验收面板、Release Notes、合并前检查文档、demo:freeze-check、UI/触控/闭环复核。
- V1.9：PWA 配置、平板安装提示、横屏提示、启动页、PWA / 平板诊断、Windows 一键启动脚本、tablet-install-guide。
- V2.0：数据导入中心、Excel/CSV 预览校验、模板下载、导入历史、导入演示文件和 import-flow 检查。
- V2.1：资料维护中心、客户/产品/计划/前段/后段/文件资料维护、复核队列、维护历史和 maintenance-flow 检查。
- V2.2：本地 Mock 登录、角色权限、权限矩阵、菜单权限、后端权限守卫和 auth-flow 检查。
- V2.3：治具库、异常库、质量标准库、现场知识页签、知识库导入、知识库权限和 knowledge-flow 检查。

## 当前仍为 Mock

- 后端业务数据仍为 Mock / 本地 metadata。
- 文件存储仍为本机 `apps/api/storage/uploads`。
- 企业微信微盘未接入。
- 真实语音识别未接入。
- Sealos PostgreSQL 未连接，未执行写库。
- 网络诊断只做 API / 文件服务只读检查，不做数据库连接。
- 演示状态重置只清理浏览器 UI 状态，不清理后端上传文件或 metadata。
- V1.7 演示数据管理面板只提供说明和前端 UI 状态重置，不做危险清理。
- V1.8 冻结前验收面板只写入本机 localStorage，不连接数据库，不伪造业务通过。
- V1.9 PWA 只预缓存前端静态资源，不离线缓存 API 和文件流。
- V2.0 导入数据只写入本地 metadata，不连接数据库，不上传企业微信微盘。
- V2.1 资料维护只写入本地 Mock / metadata 和 `maintenance-records.json`，不连接数据库，不执行真实写库。
- V2.2 角色权限仍为本地 Mock，不接企业微信登录，不做真实组织架构。
- V2.3 知识库仍为 Mock seed + 本地 metadata，不连接数据库，不接企业微信微盘。

## 安全状态

- `.env.local` 不提交。
- 上传文件不提交。
- metadata JSON 不提交。
- demo-upload-assets 只包含合成演示文件，可提交用于本地测试。
- demo-import-files 只包含合成导入文件，可提交用于本地测试。
- maintenance-records.json 属于本地维护历史，不提交。
- knowledge-*.json 属于本地知识库 metadata，不提交。

## 下一步建议

建议先完成“演示工具 → 数据导入中心”的导入走查，再进入“演示工具 → 资料维护中心”验证客户、产品、计划、参数、资料包、文件资料、复核队列和维护历史。

## V2.3 更新

当前版本：V2.3 现场知识库演示版。

已完成：
- 治具库、异常库、质量标准库 Mock API。
- 平板主界面“现场知识库”区域。
- 资料维护中心“现场知识库”页。
- 导入中心新增三类知识库模板、预览和应用。
- 全局搜索联动知识库结果。
- 知识库权限与维护历史。
- `demo:knowledge` 和 `knowledge-flow:check`。

仍为 Mock：
- 不接 Sealos PostgreSQL。
- 不接企业微信微盘。
- 不接真实语音识别。
- 不执行数据库连接或写库操作。

## V2.2 更新

当前版本：V2.2 角色权限演示版。

已完成：
- 本地 Mock 登录页 `/login`。
- 前段组长、后段组长、资料维护、工艺、品质、管理员 6 类角色。
- 前端当前用户状态、角色切换、退出登录。
- 菜单入口和按钮动作按权限显示、禁用或提示。
- 后端 Mock API 权限守卫。
- 确认、反馈、上传、导入、维护、复核等动作使用当前 Mock 用户写入操作人。
- `auth-flow:check` 只读检查脚本。

仍为 Mock：
- 不接 Sealos PostgreSQL。
- 不接企业微信微盘。
- 不接企业微信登录。
- 不接真实语音识别。
- 不做真实组织架构。
- 不执行数据库连接或写库操作。
